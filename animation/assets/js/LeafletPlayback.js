...
L.Playback.Track = L.Class.extend({
        ...
        initialize : function (geoJSON, options) {
            options = options || {};
            var tickLen = options.tickLen || 10000;
...
L.Playback.TrackController = L.Class.extend({
    ...
    tock : function (timestamp, transitionTime) {
        for (var i = 0, len = this._tracks.length; i < len; i++) {
            ...
            var lngLat0 = this._tracks[i].getFirstTick();
            var latLng = new L.LatLng(lngLat[1], lngLat[0]);
            var latLng0 = new L.LatLng(lngLat0[1], lngLat0[0]);
            this._tracks[i].moveMarker(latLng, transitionTime, timestamp);
/*            if (this._tracks[i].lineLayer0) {
		this._map.removeLayer(this._tracks[i].lineLayer0);
		this._tracks[i].lineLayer0 = null;
            }*/
/*            if (this._tracks[i].lineLayer1) {
		this._map.removeLayer(this._tracks[i].lineLayer1);
		this._tracks[i].lineLayer1 = null;
            }*/
            if (this._tracks[i].trackPresentAtTick(timestamp)) {
		if (!this._tracks[i].lineLayer0) {
                  this._tracks[i].lineLayer0 = L.polyline([[lngLat0[1], lngLat0[0]], [lngLat[1], lngLat[0]]], {color: 'black', weight:3, opacity: 1}).addTo(this._map);
                  this._tracks[i].lineLayer1 = L.polyline([[lngLat0[1], lngLat0[0]], [lngLat[1], lngLat[0]]], {color: 'white', weight:2, opacity: 1, dashArray : '3, 3'}).addTo(this._map);
                }
                else {
                   this._tracks[i].lineLayer0.setLatLngs([latLng0, latLng]);
                   this._tracks[i].lineLayer1.setLatLngs([latLng0, latLng]);
                }
            }
            else {
              if (this._tracks[i].lineLayer0) {
		...
		this._tracks[i].lineLayer1 = null;
           ...
           if (timestamp >= this._tracks[i].getEndTime() && !this._tracks[i].arrivalMarkerLayer){
              var tr = this._tracks[i];

              var lastLngLat = this._tracks[i].getLastTick();
              var arrivalMarkerLayer = L.circleMarker([lastLngLat[1], lastLngLat[0]], { radius: 3, color: 'black', fillColor: 'white'}).addTo(this._map);
              var date = new Date(timestamp);
              var hours = date.getHours();
              var minutes = date.getMinutes()
              var arrival = this._tracks[i]._geoJSON.properties.arrival.stop.properties.name;
              arrivalMarkerLayer.bindLabel(arrival+" "+hours+":"+(minutes < 10 ? '0' : '') +minutes, {direction: 'right', opacity: 0.9});
              if (timestamp - this._tracks[i].getEndTime() <= 1200000) {
                  arrivalMarkerLayer.showLabel();
              }
              this._tracks[i].arrivalMarkerLayer = arrivalMarkerLayer;
           } else if (this._tracks[i].arrivalMarkerLayer) {
                if (timestamp < this._tracks[i].getEndTime()) {
		 this._map.removeLayer(this._tracks[i].arrivalMarkerLayer);
		 this._tracks[i].arrivalMarkerLayer = null;
                } else if (timestamp - this._tracks[i].getEndTime() > 1200000) {
                    this._tracks[i].arrivalMarkerLayer.hideLabel();
                } else {
                    this._tracks[i].arrivalMarkerLayer.showLabel();
                }
           }
        }
    },
